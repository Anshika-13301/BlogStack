...
const BlogDetails = () => {
  ...
  useEffect(() => {
    fetch(`/api/blog-details-page/${id}`)
      .then((res) => {
        return res.json();
      })
      .then((data) => {
  ...
  function handleForm(e) {
    ...
    const formData = { name, comment, title };
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-20 px-3">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-6">
          <img
            src={`http://localhost:5000/uploads/${blogs.blogImage}`}
            alt={blogs.title}
        ...
        <div className="mt-4 space-y-4 text-gray-700 leading-relaxed text-sm">
          ...
          <div>
            ...
            <form action="" onSubmit={handleForm}>
              ...
              ></textarea>
              <button
                className="mt-3 px-5 py-2 bg-blue-500 text-white rounded-md cursor-pointer"
                type="submit"
              >
                Submit
              </button>
            ...
            <div className="mt-6 space-y-4">
              {allComments.map((cmnt) => (
                <div className="bg-white p-4 rounded-md shadow-sm">
                  ...
                  <p className="text-sm font-semibold">{cmnt.userName}</p>

                  <p className="mt-2 text-sm">{cmnt.userComment}</p>
                </div>
              ))}
            </div>
          </div>
